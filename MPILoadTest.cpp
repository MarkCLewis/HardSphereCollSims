...
class MySpaceMeasure {
	public:
		typedef ParallelBoundary<SpecificFixedPeriodic,MySpaceMeasure> BoundType;
		void setTree(ParallelGravCollTree<BoundType> *t) {
			tree=t;
		}
		double getMinSpacing() {
			return tree->getMinSpacing();
		}
	private:
		ParallelGravCollTree<BoundType> *tree;
};
...
//	double minx=-0.00001,maxx=0.00001,miny=-0.0003,maxy=0.0003;
	double minx=-0.000001,maxx=0.000001,miny=-0.0003,maxy=0.0003;
...
//	Boundary bc(sbc,minx,maxx,miny,maxy,pc,collForce);
	...
	typedef ParallelBoundary<SpecificBC,MySpaceMeasure> Boundary;
...
//	typedef DoubleForce<ParticleMoonForcing,CollForcing> Forcing;
	...
	typedef TreeCollisionForcing<ParallelGravCollTree<Boundary> > Forcing;
	MySpaceMeasure mySpacing;
	Boundary bc(sbc,minx,maxx,miny,maxy,pc,mySpacing);

//	typedef DoubleForce<ParallelGravCollTree<Boundary>,CollForcing> Forcing;

	ParallelGravCollTree<Boundary> gt(0.3,pc,bc);
	mySpacing.setTree(&gt);
	Forcing force(gt);
...
//	TauRandomSquareEIOrbitsWithCheck<Boundary> distrib(tau,bc.getMinX(),bc.getMaxX(),bc.getMinY(),bc.getMaxY(),eValue,iValue,rd,bc);
	UnbalancedTauRandomSquareEIOrbitsWithCheck<Boundary> distrib(tau,bc.getMinX(),bc.getMaxX(),bc.getMinYTotal(),bc.getMaxYTotal(),eValue,iValue,rd,bc);
